...
class Ui(vdebug.ui.interface.Ui):
    ...
    def open(self):
        ...
        try:
            ...
            self.outputwin = OutputWindow(self, 'below new')
            self.outputwin.create()
...
class OutputWindow(Window):
    """window that runs VimShell and dbgp, also use as output window in debugging"""

    def on_create(self):
        self.outputwin_height = int(vim.eval('&lines')) / 5
        self.outputwin_height = self.outputwin_height > 20 and 20 or self.outputwin_height
        vim.command('silent resize %d' % self.outputwin_height)
        vim.command('VimShell')
        if self.creation_count == 1:
            cmd = 'silent! au BufWinLeave %s :silent! bdelete %s' % (self.name, self.name)
            vim.command('%s | python debugger.runner.ui.outputwin.is_open = False' % cmd)
